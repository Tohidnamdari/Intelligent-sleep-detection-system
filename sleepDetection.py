import cv2
import cvzone
from FaceMeshModule import FaceMeshDetector
from cvzone.PlotModule import LivePlot

cap = cv2.VideoCapture(0,cv2.CAP_DSHOW)
detector = FaceMeshDetector(maxFaces=1)
plotY = LivePlot(640, 360, [20, 50], invert=True)

idList = [22, 23, 24, 26, 110, 157, 158, 159, 160, 161, 130, 243]
ratioList = []
blinkCounter = 0
counter = 0
color = (255, 0, 255)

while True:

    if cap.get(cv2.CAP_PROP_POS_FRAMES) == cap.get(cv2.CAP_PROP_FRAME_COUNT):
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)

    success, img = cap.read()
    img, faces = detector.findFaceMesh(img, draw=False)

    if faces:
        face = faces[0]
        for id in idList:
            cv2.circle(img, face[id], 1,color, cv2.FILLED)

        leftUp = face[159]
        leftDown = face[23]
        leftLeft = face[130]
        leftRight = face[243]
        lenghtVer, _ = detector.findDistance(leftUp, leftDown)
        lenghtHor, _ = detector.findDistance(leftLeft, leftRight)


        cv2.line(img, leftUp, leftDown, (0, 200, 0), 1)
        cv2.line(img, leftLeft, leftRight, (0, 200, 0), 1)

        ratio = int((lenghtVer / lenghtHor) * 100)

        ratioList.append(ratio)
        if len(ratioList) > 10:
            ratioList.pop(0)
        ratioAvg = sum(ratioList) / len(ratioList)
        if ratioAvg > 35.5:
	        counter=0
        if ratioAvg < 35.5 and counter == 0:
            color = (0,200,0)
            counter = 1
        if counter != 0:
            counter += 1
            if counter > 30:
                color = (255,0, 255)
                img = cv2.putText(img, 'Sleep', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                              (0, 0, 255), 3, cv2.LINE_AA, False)

        if counter < 30:
                img = cv2.putText(img, 'Not Sleep', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                  (0, 255, 0), 3, cv2.LINE_AA, False)
        imgPlot = plotY.update(ratioAvg)
        img = cv2.resize(img, (640, 360))
        imgStack = cvzone.stackImages([img, imgPlot], 2, 1)
    else:
        img = cv2.resize(img, (640, 360))
        imgStack = cvzone.stackImages([img, img], 2, 1)
    print(counter)
    cv2.imshow("Image", imgStack)
    cv2.waitKey(25)